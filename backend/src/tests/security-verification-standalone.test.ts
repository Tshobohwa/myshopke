/**
 * Security Measures Verification - Standalone Tests
 * These tests verify that security measures remain intact with weak passwords
...
describe("Security Measures Verification - Standalone Tests", () => {
  ...
  describe("Security Features Verification", () => {
    it("should verify that password validation is properly disabled", () => {
      // Test various weak password scenarios
      const testCases = [
        { password: "123", description: "numeric only" },
        ...
        { password: "abc", description: "letters only" },
        { password: "12345678", description: "numeric sequence" },
      ];
    ...
    it("should verify that bcrypt hashing works with all password types", async () => {
      const testPasswords = [
        "123",
        "a",
        "password",
        ...
        "StrongPassword123!",
        "weak",
        "12345678901234567890", // Long password
      ...
      for (const password of testPasswords) {
        ...
        // Verify hash format
        expect(hashed).toMatch(/^\$2a\$12\$.{53}$/);

        // Verify password verification works
        const isValid = await AuthUtils.verifyPassword(password, hashed);
        expect(isValid).toBe(true);

        // Verify wrong password fails
        const isInvalid = await AuthUtils.verifyPassword(
          password + "wrong",
          hashed
        );
    ...
    it("should verify JWT token security properties", () => {
      const payload = {
      ...
      const decodedAccess = jwt.decode(accessToken, { complete: true }) as any;
      ...
      expect(decodedAccess.header.typ).toBe("JWT");
      expect(decodedRefresh.header.alg).toBe("HS256");
      expect(decodedRefresh.header.typ).toBe("JWT");

      // Verify token payloads
      expect(decodedAccess.payload.userId).toBe(payload.userId);
      expect(decodedAccess.payload.email).toBe(payload.email);
      expect(decodedAccess.payload.role).toBe(payload.role);
      expect(decodedAccess.payload.iss).toBe("myshopke-api");
      expect(decodedAccess.payload.aud).toBe("myshopke-app");

      expect(decodedRefresh.payload.userId).toBe(payload.userId);
      expect(decodedRefresh.payload.email).toBe(payload.email);
      expect(decodedRefresh.payload.role).toBe(payload.role);
      expect(decodedRefresh.payload.iss).toBe("myshopke-api");
      expect(decodedRefresh.payload.aud).toBe("myshopke-app");

      // Verify expiry times are different
      expect(decodedAccess.payload.exp).not.toBe(decodedRefresh.payload.exp);
      expect(decodedRefresh.payload.exp).toBeGreaterThan(
